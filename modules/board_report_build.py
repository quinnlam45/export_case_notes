...
report_sections = {
    "cases_opened" : ["Service area", "Referrer", "Referrer report vers", "Gender", "Age group", "Ethnicity", "Sexual orientation", "Disabled", "Caring status"],
    "cases_closed" :  {"Engagement outcome": ["Advice and guidance", "One to one support"]}
    }

start_range = '1 apr 2021'
end_range = '31 mar 2022'
...
def write_to_excel_multiple_sheets(file_obj):
    ...
    with pd.ExcelWriter(file_obj) as writer:
        ...
        for service_group, groupID in service_groups.items():
...
def write_to_excel_one_report(file_obj):
    wb = Workbook()
    ws = wb.active
    with pd.ExcelWriter(file_obj) as writer:
        writer.book = wb
...
write_to_excel_one_report('main_summary.xlsx')